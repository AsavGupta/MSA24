def main():
    ...
    assignment = 0
    for i in range(0, len(gradebook[0])):
        assignment_total = 0
        for student in gradebook:
            assignment_total += student[assignment]
        print(f'Assignment {assignment+1}: {assignment_total/num_of_students:.2f}%')
        assignment+=1
    ...
    for student in gradebook:
        ...
        for i in student:
            student_total += i
        print(f'Student {student_num+1}: {student_total/len(gradebook[student_num]):.2f}%')
        student_num += 1
